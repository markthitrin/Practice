...
ll recur1(node* root,int left,int right,ll x,ll y,ll min_value = MININT,ll max_value = MAXINT) {
    if(right - left == 0) return 0;
    if(x <= min_value && max_value <= y) {
        return right - left;
    }
...
ll get_count(node* root,int left,int right,ll x,ll y) {
...
bool run_test() {
    ...
    for(int q = 0;q < a.size();q++) {
        a[q] = get_rand_value() % 200;
        p[q] = q;
    }
    node* root = built_wavelet(a,p);
    auto do_check = [&](int left,int right,ll x,ll y) {
        int ans = get_count_brute_force(a,left,right,x,y);
        int get_ans = get_count(root,left,right,x,y);
        return ans == get_ans;
    };
    for(int q = 0;q < 3000;q++) {
        ...
        ll x = get_rand_value() % 200;
        ll y = get_rand_value() % 200;;
        if(x > y) std::swap(x,y);
        if(!do_check(left,right,x,y)) 
            return false;
    }
    return true;
}
